...
func benchmarkSign(sigType string) map[string]interface{} {
	...
	if sigType == "anon" {
		...
		for _, i := range keys {
			results["sign"][fmt.Sprintf("%d", i)] = testing.Benchmark(func(b *testing.B) {
				_, scheme, _, privates, _, _ := PrepareBLS(i)
				BenchSign(b, scheme, benchMessage, privates)
			})
		}
		...
		for _, i := range keys {
			results["verify"][fmt.Sprintf("%d", i)] = testing.Benchmark(func(b *testing.B) {
				suite, scheme, publics, _, msgs, sigs := PrepareBLS(i)
				BLSBenchVerify(b, sigs, scheme, suite, publics, msgs)
			})
		}
	}
